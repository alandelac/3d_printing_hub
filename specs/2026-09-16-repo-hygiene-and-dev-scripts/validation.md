...
| R2 | `Push-Location src/3DPrintingHub.Client; npm test; Pop-Location` | **1 of 2 specs fails — pre-existing, not caused by this branch.** `app.spec.ts > should render title` asserts an `h1` containing "Hello, frontend-app" that the template never renders now that the shell is the login/dashboard UI. `git diff main..HEAD -- src/3DPrintingHub.Client` is empty, so no client file was touched. Repairing the stale baseline spec is Phase 3 work. |
